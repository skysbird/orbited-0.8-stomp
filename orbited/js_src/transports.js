...
transports.jsonp = Class(baseTransport, function(supr) {
    ...
    this._request = function(req) {
            ...
            var rType = req.type;
        	var ifr = this._ifr[rType];
    		var win = ifr.contentWindow, doc = win.document, body = ifr.contentWindow.document.body;
            if (!body){return $setTimeout(bind(this,'_request',req),100); }
			var completed = false;
			var onFinish = win[req.ebName] = function(scriptTag) {
			...
			win[req.cbName] = function callback() {
			...
			if(BrowserDetect.isWebKit) {
				...
				doc.write('<scr'+'ipt>'+req.ebName+'(false)</scr'+'ipt>');
			} else {
